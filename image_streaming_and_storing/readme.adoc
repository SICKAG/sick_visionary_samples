...
[source,bash]
----
python3 image_streaming_and_storing/python/continuous_streaming.py -i192.168.1.10 -n1 -dVisionary-S
# or
python3 image_streaming_and_storing/python/continuous_streaming.py -dVisionary-S -i192.168.1.10 -tUDP -r192.168.1.100 -s80 -n1
----

**Snapshots**
[source,bash]
----
python3 image_streaming_and_storing/python/snapshots.py -i192.168.1.10 -n1 -dVisionary-S
# or
python3 image_streaming_and_storing/python/snapshots.py -dVisionary-S -i192.168.1.10 -tUDP -r192.168.1.100 -s80 -n1
----

**External trigger**
[source,bash]
----
python3 image_streaming_and_storing/python/external_trigger.py -i192.168.1.10 -n1 -dVisionary-S
# or
python3 image_streaming_and_storing/python/external_trigger.py -dVisionary-S -i192.168.1.10 -tUDP -r192.168.1.100 -s80 -n1
----

[NOTE]
====
For Python samples, you can select point cloud file type and format:

* `--pointcloud_output pcd|ply|both` selects output file type(s)
* `--pointcloud_format ascii|binary` selects encoding for both PCD and PLY
...
----
# Binary PCD
python3 image_streaming_and_storing/python/snapshots.py -i192.168.1.10 -n1 -dVisionary-S --pointcloud_output pcd --pointcloud_format binary
----

[source,bash]
----
# Binary PLY
python3 image_streaming_and_storing/python/snapshots.py -i192.168.1.10 -n1 -dVisionary-S --pointcloud_output ply --pointcloud_format binary
----

[source,bash]
----
# Binary PCD and PLY
python3 image_streaming_and_storing/python/snapshots.py -i192.168.1.10 -n1 -dVisionary-S --pointcloud_output both --pointcloud_format binary
----
====
...
[IMPORTANT]
====
Info for customers (Linux): For stable UDP streaming, make sure the OS receive buffer limit is large enough. The sample configures `SO_RCVBUF` in the C++ networking layer, but Linux will cap this value by `net.core.rmem_max`.
...
Temporarily set to 4 MiB (until reboot):
...
If you increase the internal receive buffer in code (for example to 10 MiB), set `net.core.rmem_max` accordingly (for example `10485760`) to avoid dropped frames or receive issues.
====
